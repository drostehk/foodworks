...
class FoodShareReport(object):
    ...
    def get_report_dates(self):
        ...
        period_index  = min_index([d - datetime.now().date() for d in self.REPORT_PERIOD_ENDDATES])
    ...
    def generate_report(self, ngo, programme):
        ...
        print('GENERATING REPORT FOR PERIOD\n ', self.REPORT_STARTDATE, '-', self.REPORT_ENDDATE)
...
# Data Processing 
    ...
    def generate_report_rows(self, df_map):
        ...
        df.loc[len(df)+1] = self.genRow('Total beneficiaries', df_d.groupby('month').Distribution_Count.agg(['sum'])['sum'])

        df.loc[len(df)+1] = self.genRow('Compost volume (kg)', df_p.groupby('month').compost.agg(['sum'])['sum'])
        df.loc[len(df)+1] = self.genRow('Disposal volume (kg)', df_p.groupby('month').disposal.agg(['sum'])['sum'])
        df.loc[len(df)+1] = self.genRow('Storage volume (kg)', df_p.groupby('month').storage.agg(['sum'])['sum'])
        ...
        df.loc[len(df)+1] = ['Disposal Percentrage (%)'] + [f / a for f, a in zip(self.getList(df, 'Disposal volume (kg)'), self.getList(df, 'Total volume of food collected (kg)'))]
    ...
    def report_to_excel(self, report, ngo, programme):
        name = ".".join([ngo, programme, str(self.REPORT_ENDDATE)])
        dest_xlsx = self.REPORT_FOLDER + ".".join([name, 'report', 'xlsx'])
